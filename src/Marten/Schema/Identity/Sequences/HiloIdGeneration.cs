...
namespace Marten.Schema.Identity.Sequences
{
    public class HiloIdGeneration: IIdGeneration
    {
        ...
        public void GenerateCode(GeneratedMethod method, DocumentMapping mapping)
        {
            ...
            {
                method.Frames.Code($"if ({{0}}.{mapping.IdMember.Name} <= 0) {{0}}.Id = {{1}}.Sequences.SequenceFor({{2}}).NextInt();", document, Use.Type<ITenant>(), mapping.DocumentType);
            }
            else
            {
                method.Frames.Code($"if ({{0}}.{mapping.IdMember.Name} <= 0) {{0}}.Id = {{1}}.Sequences.SequenceFor({{2}}).NextLong();", document, Use.Type<ITenant>(), mapping.DocumentType);
            }
